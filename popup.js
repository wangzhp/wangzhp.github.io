...
function executeScriptToCurrentTab(code)
{
 ...
 return false;
	...
	function getTipSpan(color,desc){
	 ...
	 function getLunarDate(){
	  ...
	  $.ajax({
		 ...
		 async: false,
         success: function(obj){
			  if(obj&&obj.success&&obj.time&&obj.time.length==3){
				result= ' '+obj.time[1];
		   }
	  }});
	   return result;
	...
	$("#noImgBtn").bind('click',function(){
		...
		hideAllBtn();
		 //追加水印
		  $('.baiDuHotSearchNews').find('div:last').after("<div style='text-align:center;margin-top:2px;;font-size:15px;color:rgb(64, 118, 0)'>源自公众号【每日热搜榜】</div>");
	});
